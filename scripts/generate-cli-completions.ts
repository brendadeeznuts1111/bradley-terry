...
class DisposableTempDir extends String {
	...
	[Symbol.asyncDispose]() {
		return import("node:fs/promises").then((m) =>
			m.rm(`${this}`, { recursive: true, force: true }),
		);
	}
}
...
function checkGetCompletes(cwd: string): CompletionData["bunGetCompletes"] {
	...
	console.log(
		`✅ \`bun getcompletes\` available (${working.length}/${Object.keys(subcommands).length} subcommands: ${working.join(", ")})`,
	);
	return {
		available: true,
...
function addDocumentedFlags(commands: Record<string, CommandInfo>): void {
	...
	> = {
		...
		create: [
			...
			{
				name: "no-install",
				description: "Skip installing node_modules & tasks",
				hasValue: false,
			},
			{
				name: "no-git",
			...
			{
				name: "open",
				description: "Start & open in-browser after finish",
				hasValue: false,
			},
		],
		upgrade: [
			{
				name: "canary",
				description: "Upgrade to the latest canary build",
				hasValue: false,
			},
			{
				name: "stable",
				description: "Switch from canary back to the latest stable release",
				hasValue: false,
			},
		],
	};
	...
	const documentedPositionalArgs: Record<
		string,
		Array<{
			name: string;
			description: string;
	...
	if (pmPkg?.subcommands) {
		...
		> = {
			get: [
				{
					name: "key",
					description: "Package.json key(s) to get",
					required: false,
					multiple: true,
					type: "string",
					completionType: "none",
			...
			set: [
				{
					name: "key=value",
					description: "Package.json key=value pair(s) to set",
					required: true,
					multiple: true,
					type: "string",
					completionType: "none",
			...
			delete: [
				{
					name: "key",
					description: "Package.json key(s) to delete",
					required: true,
					multiple: true,
					type: "string",
					completionType: "none",
...
function generateCompletions(cliArgs: CliArgs): void {
	...
	for (const commandName of mainCommands) {
		...
		if (helpText.trim()) {
			...
			console.warn(
				`⚠️  No help output for "${commandName}" — skipping (command may be internal or undocumented).`,
			);
		}
	}
