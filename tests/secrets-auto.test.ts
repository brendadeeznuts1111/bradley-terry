...
import { envVarName, lookupEnv, secretEnvVarName } from "../src/secrets/env-key.js";
...
describe("secrets env-key", () => {
	it("builds legacy env var names (PR #2 style)", () => {
		expect(envVarName("secretclient-test", "api-key")).toBe("SECRETCLIENT_TEST_API_KEY");
	});

	it("builds SECRET_* prefixed env var names", () => {
		expect(secretEnvVarName(MASSEY_NAMESPACE, MASSEY_SECRET_NAME)).toBe(
