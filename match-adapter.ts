...
import { SqliteLoader, type GetMatchesOptions, type MatchRow } from "./sqlite-loader";
import {
  MatchSchema,
  type Match,
  type EntityId,
...
export const MatchAdapter = {
  ...
  fromMatchRow: (row: MatchRow): Effect.Effect<Match, never> =>
    Effect.gen(function* () {
      ...
      const winner: EntityId =
        row.winner_idx === 0 ? homeTeam : awayTeam;
      const loser: EntityId =
        row.winner_idx === 0 ? awayTeam : homeTeam;
