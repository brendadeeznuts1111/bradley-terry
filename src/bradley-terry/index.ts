...
): { ratings: Map<string, number>; iterations: number; convergenceDelta: number; logLikelihood: number } {
  ...
  BradleyTerry.of({
    fit: (matches, configOverride) =>
      Effect.gen(function* () {
        ...
        if (compCount > 1) {
          ...
          fitEntities = entities.filter((e) =>
            largestComponentEntities.has(e)
          );
        ...
        // Check convergence
        if (result.convergenceDelta > config.tolerance && result.iterations >= config.maxIterations) {
          warnings.push(
    ...
    predictWinProbability: (ratings, a, b) =>
      Effect.gen(function* () {
        ...
        if (sA === undefined) {
          return yield* Effect.fail(
            new EntityNotFoundErrorCtor({ entity: a })
          );
        }
        if (sB === undefined) {
          return yield* Effect.fail(
            new EntityNotFoundErrorCtor({ entity: b })
          );
        }
