...
_bun() {
	...
	local global_flags="--silent --elide-lines -v --version --revision -F --filter -b --bun --no-orphans --shell --workspaces --parallel --sequential --no-exit-on-error --watch --hot --no-clear-screen --smol -r --preload --require --import --inspect --inspect-wait --inspect-brk --cpu-prof --cpu-prof-name --cpu-prof-dir --cpu-prof-md --cpu-prof-interval --heap-prof --heap-prof-name --heap-prof-dir --heap-prof-md --if-present --no-install -i --install --install=auto -e --eval -p --print --prefer-offline --prefer-latest --port --conditions --fetch-preconnect --experimental-http2-fetch --experimental-http3-fetch --max-http-header-size --dns-result-order --dns-result-order=verbatim --dns-result-order=(default) --dns-result-order=ipv4first --dns-result-order=ipv6first --expose-gc --no-deprecation --throw-deprecation --title --zero-fill-buffers --use-system-ca --use-openssl-ca --use-bundled-ca --redis-preconnect --sql-preconnect --no-addons --unhandled-rejections --unhandled-rejections=strict --console-depth --user-agent --cron-title --cron-period --env-file --no-env-file --cwd -c --config -h --help"
