...
global_flags=(
	...
	'--elide-lines[Number of lines of script output shown when using --filter (default: 10). Set to 0 to show all lines.]:elide-lines:'
	...
	'--cpu-prof-interval[Specify the sampling interval in microseconds for CPU profiling (default: 1000)]:cpu-prof-interval:'
	...
	'--max-http-header-size[Set the maximum size of HTTP headers in bytes. Default is 16KiB]:max-http-header-size:'
	...
	'--console-depth[Set the default depth for console.log object inspection (default: 2)]:console-depth:'
		...
		case "$cmd" in
			...
			(test)
				cmd_flags=(
					...
					'--bail[Exit the test suite after  failures. If you do not specify a number, it defaults to 1.]:bail:'
					...
					'--reporter[Test output reporter format. Available: '\''junit'\'' (requires --reporter-outfile), '\''dots'\''. Default: console output.]:reporter:'
					...
					'--max-concurrency[Maximum number of concurrent tests to execute at once. Default is 20.]:max-concurrency:'
					...
					'--parallel[Run test files in parallel using N worker processes. Implies --isolate. Defaults to CPU core count.]:parallel:'
			...
			(install)
				cmd_flags=(
					...
					'--concurrent-scripts[Maximum number of concurrent jobs for lifecycle scripts (default: 2x CPU cores)]:concurrent-scripts:'
					'--network-concurrency[Maximum number of concurrent network requests (default 48)]:network-concurrency:'
			...
			(publish)
				cmd_flags=(
					...
					'--tag[Tag the release. Default is "latest"]:tag:'
					'--otp[Provide a one-time password for authentication]:otp:'
					'--auth-type[Specify the type of one-time password authentication (default is '\''web'\'')]:auth-type:'
					'--gzip-level[Specify a custom compression level for gzip. Default is 9.]:gzip-level:'
			...
			(build)
				cmd_flags=(
					...
					'--allow-unresolved[Allow unresolved dynamic import()/require() specifiers matching these glob patterns. Use '\'''\'' for opaque specifiers. Default is '\''*'\'' (allow all).]:allow-unresolved:'
					...
					'--entry-naming[Customize entry point filenames. Defaults to "[dir]/[name].[ext]"]:entry-naming:'
					'--chunk-naming[Customize chunk filenames. Defaults to "[name]-[hash].[ext]"]:chunk-naming:'
					'--asset-naming[Customize asset filenames. Defaults to "[name]-[hash].[ext]"]:asset-naming:'
					...
					'--env[Inline environment variables into the bundle as process.env.${name}. Defaults to '\''disable'\''. To inline environment variables matching a prefix, use my prefix like '\''FOO_PUBLIC_*'\''.]:env:'
